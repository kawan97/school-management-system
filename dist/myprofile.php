...
if(isset($_POST['submit'])) {
     $blood=addslashes((htmlentities($_POST['blood'])));
     $revenue=addslashes((htmlentities($_POST['revenue'])));
     $hobby=addslashes((htmlentities($_POST['hobby'])));
     $interest=addslashes((htmlentities($_POST['interest'])));
     $health=addslashes((htmlentities($_POST['health'])));

     $sqldelete="DELETE FROM profile WHERE username=?;";  
     $execudelete=$pdo->prepare($sqldelete);
     $execudelete->execute((array($_SESSION['username']))); 

     $sql="insert into profile(username,revenue,hobby,interest,health,blood)values(?,?,?,?,?,?);";  
     $execu=$pdo->prepare($sql);
     $execu->execute((array($_SESSION['username'],$revenue,$hobby,$interest,$health,$blood))); 
     $pdo= null;
     $updated=true;
    }
?>
