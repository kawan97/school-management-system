...
<div class="container  py-4 ">
  ...
  while ($row = $stmt->fetch()) {
    ...
    if($count==0){
      ...
      echo '
      <form method="post" class="w-full  my-15 mx-auto max-w-sm bg-gray-100 rounded-xl">
        <div class=" border-b  rounded-xl border-gray-500 py-2">
         <span  class="w-full text-gray-700 mx-3 py-1 px-2 leading-tight">'.$row['studentname'].' 
         ...
         <button type="submit" name="true'.$row['id'].'"  class="flex-shrink-0 sm:-my-6 md:-my-6 border-transparent border-4 bg-green-500 mx-4 text-white hover:text-green-800 text-sm py-1 px-2 rounded" >
    ...
    if($count!= 0){
      while ($row = $stmtcheck->fetch()) { 
       echo '
        <div class="w-full mx-auto my-6 max-w-sm bg-gray-100 rounded-xl">
